...
function inferCustomerSegment(payload) {
  const joined = [payload.company, payload.demand, payload.budget, payload.notes].map((item) => sanitizeText(item, 500)).join(' ').toLowerCase();
  if (/老板|公司|投资|预算|进入|落地|团队|渠道|工厂|项目/.test(joined)) {
    return /团队|项目|月|监控/.test(joined) ? '小团队' : '老板/公司';
  }
  return '个人用户';
}

function recommendOffer(segment) {
  if (segment === '老板/公司') {
    return '市场进入诊断 / 本地资源对接 / 老板级落地陪跑';
  }
  if (segment === '小团队') {
    return '定制情报监控 / 小团队作战台 / 月陪跑';
  }
  return '周机会包 / 行业包 / 机会诊断会';
}
